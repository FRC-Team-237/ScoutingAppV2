...
const AddRandomMatches = count => {
	...
	[...Array(40)].map(_ => {
		let newTeam = Math.floor(Math.random()**1.5 * 9999) + 1;
		while(teams.includes(newTeam)) {
			newTeam = Math.floor(Math.random()**1.5 * 9999) + 1;
		}
		teams.push(newTeam);
	});
	...
	for(let i = 0; i < count; i++) {
		...
		const allianceTeams = {
			"red": shuffledTeams.slice(0, 3),
			"blue": shuffledTeams.slice(3, 6)
		};
		const remainingCells = {
			"red": Math.ceil((1-Math.random()**2) * 27),
			"blue": Math.ceil((1-Math.random()**2) * 27)
		};
		const filledRows = {
			"red": emptyMatrix(),
			"blue": emptyMatrix()
		}
		const winAlliance = ["red", "blue"][Math.floor(Math.random() * 2)];

		["red", "blue"].forEach(alliance => {
			const selectedTeams = [];
			allianceTeams[alliance].forEach(team => {
				const ThisTeamRows = emptyMatrix();
				const scoreCount = Math.ceil(Math.random() * remainingCells[alliance]);
				remainingCells[alliance] -= scoreCount;

				for(let c = 0; c < scoreCount; c++) {
					let cellPosition = { row: rowNames[Math.floor(Math.random() * 3)], cell: Math.floor(Math.random() * 9) };
					while(filledRows[alliance][cellPosition.row][cellPosition.cell] != 0) {
						cellPosition = { row: rowNames[Math.floor(Math.random() * 3)], cell: Math.floor(Math.random() * 9) };
					}
					const cubeOrCone = cellPosition.row == "low" ? Math.floor(Math.random() * 2) + 1 : Math.abs(cellPosition.cell % 3 - 1) + 1;
					filledRows[alliance][cellPosition.row][cellPosition.cell] = cubeOrCone * (Math.random() > 0.5 ? -1 : 1);
					ThisTeamRows[cellPosition.row][cellPosition.cell] = filledRows[alliance][cellPosition.row][cellPosition.cell];
				}
				selectedTeams.push({
					teamNumber: team,
